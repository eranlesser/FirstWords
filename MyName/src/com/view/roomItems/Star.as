package com.view.roomItems
{
	...
	public class Star extends PlayItem
	{
		private var _assetName:String;
		private var _hgt:uint;
		public function Star(space:Space,cbType:CbType,x:int,assetName:String,hgt:uint):void{
		...
		override protected function createBody(cbType:CbType,xx:int,yy:int):void{
			...
			_body.shapes.add( new Polygon( Polygon.rect(11,0,8,_hgt), Material.wood() ) );
			_body.shapes.add( new Polygon( Polygon.rect(0,_hgt-20,30,30), Material.wood() ) );
//			var s:Polygon;
//			s = new Polygon(
...
//			s.fluidEnabled = false;
		...
		{
			body.userData.graphic.x = body.position.x-10;
			if(_assetName == "transperant/star-right-only"){
				body.userData.graphic.y = body.position.y+_hgt-60;
			}else{
				body.userData.graphic.y = body.position.y+_hgt-80;
			}
